...
namespace MFlight
{
    ...
    public class MouseFlightController : MonoBehaviour
    {
        ...
        public bool invertXKeyboard;
        public bool invertYKeyboard;
        ...
        private void RotateRig()
        {
            ...
            // OVERRIDE: With keyboard as well
            float keyX = Input.GetAxis("Horizontal");
            float keyY = Input.GetAxis("Vertical");
            if (invertXKeyboard) keyX *= -1.0f;
            if (invertYKeyboard) keyY *= -1.0f;
            ...
            if (useKeyboard)
            {
                // OVERRIDE: Control the mouse aiming using keyboard instead of mouse
                mouseAim.Rotate(cam.right, keyY, Space.World);
                mouseAim.Rotate(cam.up, keyX, Space.World);
            }
            else
            {
